...
class MongoClientInterface {
    ...
    /**
     * Puts an object into a MongoDB collection. Depending on the parameters,
     * the object is either directly put into the collection
     ...
     * @param {Object} params - Additional parameters. If params.doesNotNeedOpogUpdate is true, the object is directly
     * put into the collection. If params.vFormat is provided, it is used to format the object key.
     ...
     * @param {string} [originOp='s3:LifecycleTransitionPut'] - The operation that originated this action.
     * @returns {undefined}
     */
    putObjectNoVer(collection, bucketName, objName, value, params, log, cb, originOp = 's3:LifecycleTransitionPut') {
        const key = formatMasterKey(objName, params.vFormat);
        const putFilter = { _id: key };
        if (params && !params.doesNeedOplogUpdate) {
            // If flag is true, directly put object
